...
class FedNHCS7Client(FedUHClient):
...
class FedNHCS7Server(FedUHServer):
    ...
    def run(self, **kwargs):
        ...
        for r in round_iterator:
            ...
            if r <= 10:
                ...
                k = int(np.abs(np.sqrt(2*(r-15)+1/8) - 1/2))
                residual = int((r-15) - k*(k+1)/2)
                self.active_clients_indicies = self.sort_client[residual-1] if residual > 0 else self.sort_client[k-1]
            elif 31 <= r <= 45:
                # setup_seed(r + kwargs['global_seed'])
                k = int(np.abs(np.sqrt(2*(r-30)+1/8) - 1/2))
                residual = int((r-30) - k*(k+1)/2)
                self.active_clients_indicies = self.sort_client[residual-1] if residual > 0 else self.sort_client[k-1]
            elif 46 <= r <= 60:
                # setup_seed(r + kwargs['global_seed'])
                k = int(np.abs(np.sqrt(2*(r-45)+1/8) - 1/2))
                residual = int((r-45) - k*(k+1)/2)
                self.active_clients_indicies = self.sort_client[residual-1] if residual > 0 else self.sort_client[k-1]
            # elif 46 <= r <= 50:
            #     self.active_clients_indicies = self.sort_client[r-41]
            else:
                setup_seed(r + kwargs['global_seed'])
